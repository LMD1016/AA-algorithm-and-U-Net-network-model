...
%% Initialize GPU 
gpuDevice(1); % Select GPU device (change index if multiple GPUs available)
 
%% Load and prepare initial images on GPU 
I0 = imresize(im2double(imread('I0.bmp')),  [400 400]);    % CPU operation first 
P0 = imresize(im2double(imread('P0.bmp')),  [400 400]);    % CPU operation first 
X3 = sqrt(gpuArray(I0)) .* exp(1i * gpuArray(P0));       % Create complex field on GPU 
[a, b] = size(X3);
...
%% Create aperture mask (pupil function) on GPU 
[x, y] = meshgrid(gpuArray(-fix(M/2):ceil(M/2)-1), gpuArray(-fix(N/2):ceil(N/2)-1));
[~, R] = cart2pol(x, y);
Aperture = ~(R > kmax);         % Binary aperture mask 
Aperture_fun = double(Aperture);% Convert to double 
 
%% Initialize image acquisition sequence 
Image_num_index = zeros(1, LED_num_x * LED_num_y, 'gpuArray');  % LED illumination sequence 
kxky_index = zeros(LED_num_x * LED_num_y, 2, 'gpuArray');       % Spatial frequency offsets 
...
kxky_index0 = zeros(LED_num_x, LED_num_y, 2, 'gpuArray');
for i = 1:LED_num_x 
    for j = 1:LED_num_y 
...
kxky_index_0 = zeros(LED_num_x * LED_num_y, 2, 'gpuArray');
...
% Create coordinate system on GPU 
[x, y] = size(X3);
[X, Y] = meshgrid(gpuArray(1:x), gpuArray(1:y));
 
% Preallocate GPU arrays 
RAW = zeros(x, y, LED_num_x * LED_num_y, 'gpuArray');
mask = false(x, y, LED_num_x * LED_num_y, 'gpuArray');
...
RAW_ori1 = RAW;
RAW = guiyi(RAW);  % Normalization function (should be GPU-compatible)
 
% Compute Fourier transforms of low-res images for alignment 
PhaseLR = zeros(x, y, LED_num_x * LED_num_y, 'gpuArray');
...
centers = zeros(2, 2, LED_num_x^2, 'gpuArray');
radii = zeros(2, LED_num_x^2, 'gpuArray');
bw = false(x, y, LED_num_x * LED_num_y, 'gpuArray');
 
% Fill holes in thresholded Fourier magnitudes 
for j = 1:LED_num_y * LED_num_x 
    bw(:,:,j) = imfill(gather(NPhaseLR(:,:,j)), 'holes'); % imfill not GPU-compatible, so gather first 
    bw(:,:,j) = gpuArray(bw(:,:,j));
end 
 
% Find circles in Fourier transforms 
kxky_indexC0 = zeros(LED_num_x^2, 2, 'gpuArray');
for ix = 1:LED_num_x^2 
    try 
        % imfindcircles not GPU-compatible, so gather data first 
        [c, r] = imfindcircles(gather(NPhaseLR(:,:,ix)), [40,60], 'Sensitivity', 0.95);
        centers(:,1:2,ix) = gpuArray(c(:,:));
        radii(:,ix) = gpuArray(r(1:2));
...
kxky_indexC = zeros(size(kxky_indexC0), 'gpuArray');
...
while mae_err > 0 
    ...
    xshift = de * (rand(1,3, 'gpuArray')) .* [-1, 1, 0];
    yshift = de * (rand(1,3, 'gpuArray')) .* [-1, 1, 0];
    rotation = de1 * (rand(1,3, 'gpuArray')) .* [-1, 1, 0];
    ...
    for Nshift = 1:3 
        ...
        kxky_index1 = zeros(LED_num_x, LED_num_y, 2, 'gpuArray');
        for i = 1:LED_num_x 
            for j = 1:LED_num_y 
        ...
        kxky_indextest = zeros(LED_num_x * LED_num_y, 2, 'gpuArray');
        ...
        RAW1 = zeros(x, y, LED_num_x * LED_num_y, 'gpuArray');
        for i = 1:LED_num_x^2 
        ...
        PhaseLR1 = zeros(x, y, LED_num_x * LED_num_y, 'gpuArray');
        for j = 1:LED_num_x^2 
...
kxky_index_cor = zeros(LED_num_x, LED_num_y, 2, 'gpuArray');
for i = 1:LED_num_x 
    for j = 1:LED_num_y 
        m = [(i-(LED_num_x+1)/2), (j-(LED_num_y+1)/2)] * [cos(theta_cor*pi/180); sin(theta_cor*pi/180)];
        n = [(i-(LED_num_x+1)/2), (j-(LED_num_y+1)/2)] * [-sin(theta_cor*pi/180); cos(theta_cor*pi/180)];
...
kxky_index_cor0 = zeros(LED_num_x * LED_num_y, 2, 'gpuArray');
...
Result = zeros(x, y, Total_iter_num, 'gpuArray');
MSE = zeros(1, Total_iter_num, 'gpuArray');
Threshold = zeros(LED_num_x * LED_num_y, Total_iter_num, 'gpuArray');
 tic
...
%             end 
%         end 
        mask_active = (mask(:,:,num) ~= 0);
        Subspecturm1(mask_active) = Subspecturmnew1(mask_active);
    ...
    subplot(1,3,1)
    imshow(gather(log(abs(F)+1)), [0, max(max(gather(log(abs(F)+1))))/2]);
    title('Fourier spectrum', 'FontSize', 20, 'FontName', 'Times New Roman');
    
    subplot(1,3,2)
    imshow(gather(abs(Result(:,:,iter))), []);
    title('Amplitude', 'FontSize', 20, 'FontName', 'Times New Roman');
    
    subplot(1,3,3)
    imshow(gather(imag(Result(:,:,iter))), []);
